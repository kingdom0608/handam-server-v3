...
export class FilterToQuery {
	...
	constructor(
		private options: FilterToQueryOptions) {
	}
	...
	parser(pageQuery: FilterToQueryPageQuery, authFilter?: object) {
		if (authFilter) {
			...
			if (pageQuery.filter
				&& Object.keys(pageQuery.filter).length > 0) {
				pageQuery.filter = {
					and: {
		...
		let {
			where,
			initIncludeAliases
		} = this.parseFilter(pageQuery.filter);
		/** 정렬 파서 */
		let order = [];
		if (pageQuery.orderBy) {
			const orderResult = this.mapOrderBy(pageQuery.orderBy, initIncludeAliases);
			initIncludeAliases = initIncludeAliases.concat(orderResult.initIncludeAliases);
	 ...
	 */
	mapOrderBy(orderBy: Array<{field: string; direction: string}>, initIncludeAliases: string[]) {
	...
	mapInclude(include: Includeable[], includeAlias: string) {
		 ...
		 */
		const addIncludes = includeAlias
			.split('.')
			.map((relationAlias) => {
				const result = this.options.include
					.find(include => include.as === relationAlias);
				if (!result) {
	...
	parseIncludeKey(key: string, includeAlias: string): string {
		const include = this.options.include.find(include => include.alias === includeAlias);
		if (!include) {
	...
	parseKey(key: string) {
		...
		/** 정의된 칼럼에서 탐색 */
		const col = this.options.columns
			.find(col => col.alias === key);
		if (!col) {
