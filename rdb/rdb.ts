...
export class Rdb {
	...
	constructor(database: string, models: Array<typeof Model>, private options: IRdbOptions) {
		const rdbConnection: IRdbOptionsConnection = options.connection;
		let prefix = (options.stage === 'local' || options.stage === 'test') ? 'local' : options.stage;
		let separator = (options.databasePrefixSeparator) ? options.databasePrefixSeparator : '-';
		prefix = (options.databasePrefix !== undefined) ? options.databasePrefix : prefix;
	...
	async sync(options: {force: boolean} = {force: false}): Promise<void> {
		if (!this.isSync) {
			if (options.force) {
				if (this.options.connection.host === 'dv' || this.options.connection.host === 'prod') {
